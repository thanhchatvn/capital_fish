...
class StockInventory(models.Model):
    ...
    @api.model
    def create_stock_inventory_ept(self, products, location_id,
                               auto_validate=False):
        """
        Added by Udit
        ...
        if products:
            ...
            inventory_vals = {
                'name': inventory_name,
                'location_ids': [
                    (6, 0, [location_id.id])] if location_id else False,
            ...
            inventory.create_inventory_lines_ept(products, location_id)
    ...
    def create_inventory_lines_ept(self, products, location_id):
        ...
        for product_data in products:
            if product_data.get('product_id', False) and product_data.get(
                    'product_qty', False):
                val = self.prepare_inventory_line_vals_ept(
