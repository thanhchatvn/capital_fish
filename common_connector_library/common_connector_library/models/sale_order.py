...
class SaleOrder(models.Model):
    ...
    moves_count = fields.Integer(compute="_compute_stock_move",
                                 string="Stock Move",
                                 store=False,
    ...
    def onchange_partner_shipping_id(self):
        ...
        if self.warehouse_id and self.partner_id.allow_search_fiscal_based_on_origin_warehouse:
            ...
            fiscal_position = self.env['account.fiscal.position'].with_context(\
                {'origin_country_ept': origin_country_id, 'is_amazon_fpos':is_amz_customer}).with_company(warehouse.company_id.id).\
    ...
    """ Added method of auto invoice workflow module code to merge with common connetor"""
    ...
    def paid_invoice_ept(self, invoices):
        ...
        for invoice in invoices:
            if invoice.amount_residual:
                vals = invoice.prepare_payment_dict(
    ...
    def auto_shipped_order_ept(self, customers_location,
        ...
        for order_line in order_lines:
            if not is_mrp_installed:
                ...
                bom_lines = self.amz_fbm_shipped_get_set_product_ept(order_line.product_id)
                for bom_line in bom_lines:
                    self.create_and_done_stock_move_ept(order_line,
                                                    customers_location,
                                                    bom_line)
                if not bom_lines:
                    self.create_and_done_stock_move_ept(order_line,
    ...
    def amz_fbm_shipped_get_set_product_ept(self, product):
        """
    ...
    def create_and_done_stock_move_ept(self, order_line, customers_location,
                                   bom_line=False):
        """
        Added by Udit
