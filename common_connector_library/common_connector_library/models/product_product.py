...
class ProductProduct(models.Model):
    ...
    ept_image_ids = fields.One2many('common.product.image.ept', 'product_id',
                                    string='Images')
    vendor_ids = fields.One2many('vendor.stock.ept', 'vendor_product_id',
                                 string="Vendor")
    is_drop_ship_product = fields.Boolean(string="Is drop ship product",
                                          store=False,
    ...
    def _compute_is_drop_ship_product(self):
        ...
        """
        customer_obj = self.env['stock.location'].search(
            [('usage', '=', 'customer')])
        ...
        stock_rule = self.env['stock.rule'].search(
            [('company_id', '=', self.env.company.id), ('action', '=', 'buy'),
             ...
             ('route_id', 'in', route_ids.ids)]).route_id
        if stock_rule:
    ...
    def get_stock_ept(self, product_id, warehouse_id, fix_stock_type=False,
                      fix_stock_value=0,
    ...
    def get_products_based_on_movement_date_ept(self, from_datetime, company=False):
        ...
        if mrp_module :
            mrp_qry = """select p.id as product_id from product_product as p
                            ...
                            where sm.date >= '%s' and sm.company_id = %d and sm.state in ('partially_available','assigned','done')"""% (date, company.id)
    ...
    def get_qty_on_hand_ept(self, warehouse, product_list):
        """
    ...
    def get_vendor_stock_ept(self):
        ...
        if product_obj:
            ...
            result = self._cr.dictfetchall()
            for i in result:
                vendor_stock.update({i.get('product_id') : i.get('stock')})
        return vendor_stock

    def get_bom_product_stock_ept(self, product_id, warehouse_id,
        ...
        if not mrp_module:
            raise UserError(_("MRP module must be installed to do this process."))
        actual_stock = product_id.find_bom_product_possible_quantity_ept(
    ...
    def find_bom_product_possible_quantity_ept(self, warehouse_id,
                                           stock_type='virtual_available'):
        """
        Added by Udit
